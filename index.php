...
foreach ($array as $user) {
  if (!$user['email']) {
    ...
    if ($user['identity_url']) {
      $countOpenID++;
      $usuarios[$user['_id']] = $user['login'].'@openid.migrate';
    } else if (preg_match('/(_twitter)/i', $user['login'])) {
      $countTwitter++;
      $username = str_replace('_twitter', '', $user['login']);
      $usuarios[$user['_id']] = $username.'@openid.migrate';
    } else {
      $usuarios[$user['_id']] = $user['login'].'@shapado.migrate';
    }
  } else {
    $usuarios[$user['_id']] = $user['email'];
  }
}
